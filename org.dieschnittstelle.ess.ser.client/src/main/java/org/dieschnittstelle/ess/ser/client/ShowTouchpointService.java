...
public class ShowTouchpointService {
    ...
    public void deleteTouchpoint(AbstractTouchpoint tp) {
        ...
        boolean async = false;
        HttpDelete httpDelete = new HttpDelete("http://localhost:8080/api/" + (async ? "async/touchpoints/" : "touchpoints/") + tp.getId());

        try {
            Future<HttpResponse> responseFuture = client.execute(httpDelete, null);
            logger.debug(responseFuture.get().getStatusLine());
    ...
    public AbstractTouchpoint createNewTouchpoint(AbstractTouchpoint tp) {
        ...
        try {
            ...
            boolean async = false;
            HttpPost httpPost = new HttpPost("http://localhost:8080/api/" + (async ? "async/touchpoints" : "touchpoints"));
            ...
            Future<HttpResponse> responseFuture = client.execute(httpPost, null);
